...
function selectCus(cus) {
    ...
    if (cus.dateofbirth) {
        $("#customerOrderForm").find('input[name="dateofbirth"]').val(stringToDate(cus.dateofbirth));
    }

    if (cus.statecode != null) {
        var $stateSelect = $($("#customerOrderForm").find('select[name="state"]')[0]);
        if($stateSelect.find('option[value="'+cus.statecode+'"]').length == 0){
            $stateSelect.append(`<option value="${cus.statecode}">${cus.stateName}</option>`);
        }
        $stateSelect.val(cus.statecode);
    }
    if (cus.city != null) {
        var $citySelect = $($("#customerOrderForm").find('select[name="city"]')[0]);
        if($citySelect.find('option[value="'+cus.city+'"]').length == 0){
            $citySelect.append(`<option value="${cus.city}">${cus.cityName}</option>`);
        }
        $citySelect.val(cus.city);
    }
    if (cus.zip != null) {
        var $zipSelect = $($("#customerOrderForm").find('select[name="zip"]')[0]);
        if($zipSelect.find('option[value="'+cus.zip+'"]').length == 0){
            $zipSelect.append(`<option value="${cus.zip}">${cus.wardName}</option>`);
        }
        $zipSelect.val(cus.zip);
    }
...
function submitCustomerOrderForm(status) {
    ...
    customer.fullname = $("#customerOrderForm").find('select[name="fullname"]').val();
    ...
    customer.dateofbirth = $("#customerOrderForm").find('input[name="dateofbirth"]').val();
    customer.dateFormat = 'dd/MM/yyyy';
    ...
    if($("#customerOrderForm").find('select[name="branchid"]').length > 0){
        requestOrder.branchid = $("#customerOrderForm").find('select[name="branchid"]').val();
    }
...
function autocomplete(inputid) {
    ...
    inp.addEventListener("input", function(e) {
        ...
        for (i = 0; i < productArray.length; i++) {
            ...
            var index = productName.toUpperCase().indexOf(val.toUpperCase());
            if (index >= 0) {
                b = document.createElement("DIV");
                b.innerHTML = productName.substr(0, index);
                b.innerHTML += "<strong>" + productName.substr(index, val.length) + "</strong>";
                b.innerHTML += productArray[i].name.substr(index + val.length);
                b.innerHTML += "<input type='hidden' value='" + i + "'>";
                b.addEventListener("click", function(e) {
                    var index = this.getElementsByTagName("input")[0].value;
    ...
    $('#cfullname').select2({
        ...
        templateSelection: function (data) {
            if (data.id === '') { // adjust for custom placeholder values
                return $('#cfullname').attr('placeholder') || '';
            }
            return data.text;
        },
        ajax: {
            url: '/bo/data/customers.json',
            type: 'GET',
            dataType: 'json',
            quietMillis: 50,
            contentType: "application/json",
            ...
            processResults: function (data) {
                return {
                    results: $.map(data, function (item) {
                        var obj = item;
                        obj.text = item.fullname +  (item.dateofbirth != null ? (" - " + stringToDate(item.dateofbirth)) : '');
                        obj.id = item.fullname
                        return obj;
                    })
                };
            },
