...
<c:if test="${destination != null}">
    <div class="d-none one-destination">
        <div class="one-destinationid">${destination.destinationID}</div>
        <div class="one-destinationname">${destination.name}</div>
        <div class="one-statecode">${destination.stateCode}</div>
        <div class="one-statename">${destination.stateName}</div>
        <div class="one-citycode">${destination.cityCode}</div>
        <div class="one-cityname">${destination.cityName}</div>
        <div class="one-nextevent">
            <c:if test="${destination.nextevent != null}">
                <fmt:formatDate value="${destination.nextevent}" pattern="dd/MM/yyyy HH:mm"/>
            </c:if>
        </div>
    </div>
</c:if>
