...
<c:choose>
    ...
    <c:otherwise>
        <section class="slice bg-gradient-primary" data-offset-top="#header-main">
            <div class="text-center">Chọn nơi mà bạn sinh sống để mua chung nhé</div>
        </section>
    </c:otherwise>
</c:choose>
    ...
    <div class="bg-dark py-2">
        <marquee behavior="scroll" direction="left" scrollamount="5">
            ...
            <c:forEach items="${nextevents}" var="desnext">
                <span class="px-3"><a class="text-warning" href="/khu-cach-ly/${desnext.nameurl}">${desnext.name} - <strong><fmt:formatDate pattern="dd/MM/yyyy" value="${desnext.nextevent}"/></strong></a></span>
            </c:forEach>
        </marquee>
    </div>
</c:if>
...
<div class="d-none" id="des-template">
    <div class="col-md-4 col-6 py-2">
        <a href="/khu-cach-ly/{code}"><i class="fas fa-map-marker-alt"></i> {name} <span class="text-warning group-count" data-groupid="{groupid}"></span></a>
    </div>
</div>