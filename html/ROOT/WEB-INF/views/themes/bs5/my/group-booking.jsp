...
<div class="mybooking">
    <div class="booking-container">
        <div class="row my-4">
            <div class="col-3">
                <h1><fmt:message key='label.group-booking'/></h1>
            </div>
            <div class="col-9 text-end">
                <security:authorize access="hasAnyAuthority('AGT_ADMIN','AGT_SUSER','AGT_USER','PM','ASSISTANT','CASHIER')">
                    <a href="#" class="btn btn-success btn-export-booking"><i class="bi bi-download"></i> Export to Exel</a>
                </security:authorize>
            </div>
        </div>
        <div class="mt-2">
            <form:form id="myBookingForm" action="/my/group-booking.html" method="post" modelAttribute="items">
                <div class="row">
                    ...
                    <div class="form-group col-md col-6">
                        <form:select path="groupid" class="form-control cus-destination">
                            <option value="0">--<fmt:message key="pickup.destination"/>--</option>
                            <c:forEach items="${destinations}" var="des">
                                <form:option value="${des.destinationID}">${des.name}</form:option>
                            </c:forEach>
                        </form:select>
                    </div>
                    <div class="form-group col-md col-6">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <c:forEach items="${items.listResult}" var="booking">
                    <tr class="tr-booking-id-${booking.customerorderid} tr-priority-${booking.priority}">
                        <td>
                            <a href="/my/booking/${booking.customerorderid}.html">${booking.ordernumber}</a>
                        </td>
                        <td class="text-end"><span class="finalamount"><fmt:formatNumber value="${booking.finalamount}" maxFractionDigits="0"/></span></td>
                        <td>${booking.groupname}</td>
                        <td>${booking.customer.fullname}</td>
                        <td><a href="javascript:void(0)" class="btn btn-link btn-open-email-form" data-bookingid="${booking.customerorderid}">${booking.customer.email}</a></td>
                        <td>${booking.customer.phone}</td>
                        <td>${booking.customer.city}</td>
                        <td>${booking.customer.address}</td>
                        <td><fmt:formatDate type="both" value="${booking.orderdate}" /></td>
                        <td class="td-status"><div class="status ${booking.status} change-status" data-status="${booking.status}" data-bookingid="${booking.customerorderid}"><span><fmt:message key="${booking.status}"/></span></div></td>
...
<security:authorize access="hasAnyAuthority('AGT_ADMIN','AGT_SUSER','AGT_USER','PM','ASSISTANT','CASHIER')">
    ...
    <div class="modal fade" id="emailModal" tabindex="-1" role="dialog" aria-labelledby="emailModalLabel">
        <div class="modal-dialog" role="document" style="width: 98%;max-width: 800px;">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form>
                        <input type="hidden" id="emailtarget" name="emailtarget" value="">
                        <input type="hidden" id="emailtargetid" name="emailtargetid" value="">
                        <div class="form-group row">
                            <label for="emailToList" class="col-sm-2 col-form-label">To</label>
                            <div class="col-sm-10">
                                <input type="text" class="form-control-plaintext" id="emailToList" name="emailToList" value="">
                            </div>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            <label for="emailContentBox" class="col-sm-2 col-form-label">Content</label>
                            <div class="col-sm-10">
                                <div id="emailContentBox" class="email-content-box" contenteditable="true">
                                    Hi
                                </div>
                            </div>
                        </div>
