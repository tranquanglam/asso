function BoCalendar(){
            ...
            eventSources: [
                // your event source
                {
                    events: function (start, end, timezone, callback) {
                        $.ajax({
                            url: "/bo/data/cal/get-calendar.json?start=" + start.unix(),
                            type: 'GET',
                            cache: false,
    ...
    this.loadCalendar = function(sdate,callback) {
        $.ajax({
            url: '/bo/data/cal/'+sdate+'/ajax.html',
            type: 'POST',
            contentType: "application/json",
            beforeSend: function(xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + ACCESSS_TOKEN);
    ...
    this.createDateAppointment = function(dateAppointment, callback) {
        $.ajax({
            url: '/bo/data/cal/create-date-appointment.json',
            type: 'POST',
    ...
    this.createAppointment = function(appointmentRequest, callback) {
        $.ajax({
            url: '/bo/data/cal/create-appointment.json',
            type: 'POST',
    ...
    this.getEnetMeetingUrl = function(appointmentid, callback) {
        $.ajax({
            url: '/bo/data/cal/'+appointmentid+'/get-enet-meetingurl.json',
            type: 'GET',
            contentType: "application/json",
            beforeSend: function(xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + ACCESSS_TOKEN);
    ...
    this.deleteAppointment = function(appointmentid, callback) {
        $.ajax({
            url: '/bo/data/cal/'+appointmentid+'/delete-appointment.json',
            type: 'POST',
            contentType: "application/json",
            beforeSend: function(xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + ACCESSS_TOKEN);
    ...
    this.bookAppointment = function(appointmentid, customer, callback) {
        $.ajax({
            url: '/bo/data/cal/'+appointmentid+'/book-appointment.json',
            type: 'POST',
    ...
    this.getAppointment = function(id,callback) {
        $.ajax({
            url: '/bo/data/cal/'+id+'/get-appointment.json',
            type: 'GET',
            contentType: "application/json",
            beforeSend: function(xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + ACCESSS_TOKEN);
...
var BO_CALENDAR = new BoCalendar();
...
function appointmentFormSubmit(){
    ...
    dateAppointment.maxPax = $('#appointmentForm .maxpax').val();
    dateAppointment.maxNumber = $('#appointmentForm .maxnumber').val();
    BO_CALENDAR.createDateAppointment(dateAppointment,function(result){
        if (result.appointments != null && result.appointments.length > 0) {
            BO_CALENDAR.reloadCalendar();
        }
    });
}
...
function addNewAppointmentSubmit() {
    ...
    BO_CALENDAR.createAppointment(appointmentRequest,function(result){
...
function showOneAppointment(ele) {
    ...
    BO_CALENDAR.getAppointment(appointmentid,function(result){
        if (result.appointmentid != null) {
...
function deleteOneAppointment(ele) {
    ...
    if (appointmentid != null && appointmentid != '') {
        BO_CALENDAR.deleteAppointment(appointmentid,function(result){
...
function gotoMeetingRoom(ele) {
    var appointmentid = $(ele).data('appointmentid');
    BO_CALENDAR.getEnetMeetingUrl(appointmentid,function(result){
        if (result != null && result.meetingurl != null) {
            window.location = result.meetingurl;
        }
    });
}
...
function addCustomerToAppointment() {
    ...
    BO_CALENDAR.bookAppointment(appointmentid,customer,function(result){
        if (result) {
            BO_CALENDAR.reloadCalendar();
        }
    });
...
$(document).ready(function() {
    ...
    BO_CALENDAR.initialize(calendarContainer,accessToken,function() {
        $('#appointment-section').addClass('show');
        bindAppointmentStatus();
    });
    $('.bd-sidebar').find('.bd-sidebar-cal').addClass('active');
});
